...
public sealed class CartOrdersControllerTests
{
    ...
    {
        var licenseResponse = new LicenseOptionsResponse { Keycode = "RESOLVED", LicenseVerified = true };
    ...
    [Fact]
    public async Task GetLicenseOptions_JsonContainsLicenseVerified()
    {
        var licenseResponse = new LicenseOptionsResponse { Keycode = "RESOLVED", LicenseVerified = true };
        _serviceMock.Setup(s => s.GetLicenseOptionsByMessageKeyAsync(ValidGuid, null, It.IsAny<CancellationToken>()))
                    .ReturnsAsync(ServiceResult<LicenseOptionsResponse>.Ok(licenseResponse));
        ...
        Assert.True(doc.RootElement.TryGetProperty("data", out var dataNode));
        Assert.True(dataNode.TryGetProperty("license_verified", out var verifiedNode));
        Assert.True(verifiedNode.GetBoolean());
    }
...
=======
    ...
    [Fact]
    public async Task GetLicenseOptions_ForwardsLocaleToService()
    {
        var response = new LicenseOptionsResponse { Keycode = "RESOLVED" };
        _serviceMock.Setup(s => s.GetLicenseOptionsByMessageKeyAsync(ValidGuid, "en-US", It.IsAny<CancellationToken>()))
                    .ReturnsAsync(ServiceResult<LicenseOptionsResponse>.Ok(response));

        var result = await CreateController().GetLicenseOptions(ValidGuid, "en-US", CancellationToken.None);

        _ = Assert.IsType<OkObjectResult>(result);
        _serviceMock.Verify(s => s.GetLicenseOptionsByMessageKeyAsync(ValidGuid, "en-US", It.IsAny<CancellationToken>()), Times.Once);
    }
...
>>>>>>> 31c99d3 (Update license options for legacy parity and product configuration)
}
